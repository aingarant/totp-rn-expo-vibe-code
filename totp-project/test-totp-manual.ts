...
async function testTOTPService() {
  ...
  console.log(
    `Invalid secret rejection: ${!isSecretInvalid ? '✅ PASS' : '❌ FAIL'}`
  );
  ...
  console.log(
    `Secret validation: ${totpService.validateSecret(generatedSecret) ? '✅ PASS' : '❌ FAIL'}`
  );
  ...
  try {
    ...
    console.log(
      `Parse success: ${parseResult.secret === testSecret ? '✅ PASS' : '❌ FAIL'}`
    );
  } catch (error) {
